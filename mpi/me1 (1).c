...
int main(int argc, char *argv[]) {
    ...
    do {
        ...
        if (prime * prime > low_value)
            first = (prime * prime +1)/2-2;
        else {
            if (!(low_value % prime)) first = 0;
            else first = (prime-low_value%prime+low_value)%2==0?(low_value+2*prime-low_value%prime)/2:(prime-low_value%prime)/2;
        }
        for (i = first; i < size; i = i + 1+ prime/2  ) marked[i] = 1;
